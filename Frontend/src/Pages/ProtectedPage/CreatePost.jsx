...
import { 
  X, ImagePlus, Eye, FileText, Save, Send, Tag, 
  Hash, Clock, Type, Image as ImageIcon, BookOpen,
  Bold, Italic, Link, List, ListOrdered, Quote,
  Code, Globe, Lock, Check, Sparkles, Loader2, 
...
export default function CreatePostPage() {
  ...
  const popularTags = [
    "react", "javascript", "webdev", "programming", "technology",
    "ai", "design", "css", "nodejs", "python", "learning", "tutorial"
  ];
  ...
  useEffect(() => {
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        // Set default profile for preview
        setUserProfile({
          username: "Your Name",
          full_name: "Your Name",
  ...
  return (
    <>
      {isPageLoading ? (
        ...
        <>
          <NavbarPrivate />
          <div className={`min-h-screen bg-gradient-to-b ${theme === 'light' ? 'from-gray-50 to-white' : 'from-slate-900 to-slate-950'} pt-20 md:pt-24`}>
            <div className="max-w-6xl mx-auto px-4 py-8">
              {/* Header */}
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
                <div>
                  <h1 className={`text-3xl md:text-4xl font-bold ${theme === 'light' ? 'text-gray-900' : 'text-white'} mb-2`}>
                    Create New Story
                  </h1>
                ...
                <div className="flex items-center gap-3">
                  {lastSaved && (
                    <div className={`hidden md:flex items-center gap-2 px-3 py-1.5 ${theme === 'light' ? 'bg-green-50 text-green-700' : 'bg-green-900/20 text-green-400'} rounded-lg text-sm`}>
              ...
              {!showPreview ? (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="lg:col-span-2 space-y-6">
                    {/* Title Input */}
                    <div className={`${theme === 'light' ? 'bg-white' : 'bg-slate-800'} rounded-2xl border ${theme === 'light' ? 'border-gray-200' : 'border-slate-700'} p-6 shadow-sm`}>
                      <div className="flex items-center justify-between mb-2">
                        <label className={`flex items-center gap-2 text-sm font-semibold ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>
                        ...
                        <div className="flex items-center gap-2">
                          {titleScore >= 3 && (
                            <span className={`flex items-center gap-1 text-xs ${theme === 'light' ? 'text-green-600' : 'text-green-400'}`}>
                      ...
                      <p className={`text-xs ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'} mt-2`}>
                        Tip: Include keywords and keep it under 80 characters for best results
                      </p>
                    </div>

                    {/* Content Editor */}
                    <div className={`${theme === 'light' ? 'bg-white' : 'bg-slate-800'} rounded-2xl border ${theme === 'light' ? 'border-gray-200' : 'border-slate-700'} shadow-sm overflow-hidden`}>
                      {/* Toolbar */}
                      <div className={`flex flex-wrap items-center gap-2 p-4 border-b ${theme === 'light' ? 'border-gray-200 bg-gray-50' : 'border-slate-700 bg-slate-900/50'}`}>
                        <span className={`text-sm font-medium ${theme === 'light' ? 'text-gray-700' : 'text-gray-300'} mr-2`}>
                          Formatting:
                        </span>
                        {[
                          { icon: <Bold className="w-4 h-4" />, cmd: 'bold', label: 'Bold' },
                          { icon: <Italic className="w-4 h-4" />, cmd: 'italic', label: 'Italic' },
                          { icon: <Link className="w-4 h-4" />, cmd: 'link', label: 'Link' },
                          { icon: <List className="w-4 h-4" />, cmd: 'list', label: 'List' },
                          { icon: <ListOrdered className="w-4 h-4" />, cmd: 'numbered', label: 'Numbered List' },
                          { icon: <Quote className="w-4 h-4" />, cmd: 'quote', label: 'Quote' },
                          { icon: <Code className="w-4 h-4" />, cmd: 'code', label: 'Code Block' },
                        ].map((item) => (
                          <button
                            key={item.cmd}
                      ...
                      <div className="p-6">
                        <textarea
                          ...
                          onChange={handleInputChange}
                          rows={15}
                          placeholder="Start writing your story here... You can use Markdown for formatting. Share your insights, experiences, and knowledge with the community!"
                      ...
                      {/* Stats Bar */}
                      <div className={`flex flex-wrap items-center justify-between gap-4 p-4 border-t ${theme === 'light' ? 'border-gray-200 bg-gray-50' : 'border-slate-700 bg-slate-900/50'}`}>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className={`${theme === 'light' ? 'bg-white' : 'bg-slate-800'} rounded-2xl border ${theme === 'light' ? 'border-gray-200' : 'border-slate-700'} p-6 shadow-sm`}>
                      <div className="flex items-center gap-2 mb-4">
                        <ImageIcon className={`w-5 h-5 ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`} />
                        <label className={`text-sm font-semibold ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>
                          Featured Image
                        </label>
                      </div>
                      ...
                      {imagePreview ? (
                        ...
                        <label className={`flex flex-col items-center justify-center w-full h-48 border-2 border-dashed ${theme === 'light' ? 'border-gray-300 hover:border-blue-500' : 'border-slate-600 hover:border-blue-400'} rounded-xl cursor-pointer transition-colors group`}>
                          <div className="flex flex-col items-center justify-center text-center p-4">
                            <ImagePlus className={`w-10 h-10 ${theme === 'light' ? 'text-gray-400 group-hover:text-blue-500' : 'text-slate-500 group-hover:text-blue-400'} mb-3`} />
                            <p className={`text-sm font-semibold ${theme === 'light' ? 'text-gray-700' : 'text-gray-300'} mb-1`}>
                              Upload Image
                            </p>
                            <p className={`text-xs ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'}`}>
                              Drag & drop or click to upload
                            </p>
                            <p className={`text-xs ${theme === 'light' ? 'text-gray-400' : 'text-slate-500'} mt-2`}>
                    ...
                    <div className={`${theme === 'light' ? 'bg-white' : 'bg-slate-800'} rounded-2xl border ${theme === 'light' ? 'border-gray-200' : 'border-slate-700'} p-6 shadow-sm`}>
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                          <Tag className={`w-5 h-5 ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`} />
                          <label className={`text-sm font-semibold ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>
                            Topics
                          </label>
                        </div>
                      ...
                      <div className="relative">
                        ...
                        {/* Tag Suggestions */}
                        {showTagSuggestions && (
                          <div className={`absolute top-full left-0 right-0 mt-1 ${theme === 'light' ? 'bg-white' : 'bg-slate-800'} border ${theme === 'light' ? 'border-gray-200' : 'border-slate-700'} rounded-lg shadow-lg z-10`}>
                      ...
                      {/* Popular Tags */}
                      <div className="mt-4">
                        <p className={`text-xs ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'} mb-2`}>
                          Popular topics:
                        </p>
                    ...
                    <div className={`${theme === 'light' ? 'bg-white' : 'bg-slate-800'} rounded-2xl border ${theme === 'light' ? 'border-gray-200' : 'border-slate-700'} p-6 shadow-sm`}>
                      <div className="flex items-center gap-2 mb-4">
                        <Send className={`w-5 h-5 ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`} />
                        <label className={`text-sm font-semibold ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>
                          Publishing Options
                        </label>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className={`font-medium ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>Public</p>
                              <p className={`text-xs ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'}`}>Anyone can see this</p>
                            </div>
                          </div>
                          <button
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className={`font-medium ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>Private</p>
                              <p className={`text-xs ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'}`}>Only you can see this</p>
                            </div>
                          </div>
                          <button
                        ...
                        <div className={`pt-4 border-t ${theme === 'light' ? 'border-gray-200' : 'border-slate-700'}`}>
                          <div className="flex items-center justify-between mb-2">
                            <label className={`font-medium ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>
                              Allow Comments
                            </label>
                            <button
                ...
                /* Preview Mode */
                <div className={`${theme === 'light' ? 'bg-white' : 'bg-slate-800'} rounded-2xl border ${theme === 'light' ? 'border-gray-200' : 'border-slate-700'} shadow-xl overflow-hidden`}>
                  {/* Preview Header */}
                  <div className={`p-8 border-b ${theme === 'light' ? 'border-gray-200' : 'border-slate-700'}`}>
                    <div className="flex items-center justify-between mb-6">
                      <div className="flex items-center gap-3">
                        <img
                          src={userProfile?.avatar_url || "https://api.dicebear.com/7.x/avataaars/svg?seed=author"}
                          alt="Author"
                        ...
                        <div>
                          <p className={`font-bold ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>
                            {userProfile?.full_name || userProfile?.username || "Your Name"}
                          </p>
                          <div className={`flex items-center gap-3 text-sm ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'}`}>
                            <span>Just now</span>
                            <span>·</span>
                    ...
                    <h1 className={`text-3xl md:text-4xl font-bold ${theme === 'light' ? 'text-gray-900' : 'text-white'} mb-4`}>
                  ...
                  <div className="p-8">
                    ...
                    <div className={`prose ${theme === 'dark' ? 'prose-invert' : ''} max-w-none`}>
                      <pre className={`whitespace-pre-wrap font-sans ${theme === 'light' ? 'text-gray-700' : 'text-gray-300'} leading-relaxed text-lg`}>
                        {formData.content || "Your story content will appear here..."}
                      </pre>
                    </div>

                    {/* Preview Footer */}
                    <div className={`mt-12 pt-8 border-t ${theme === 'light' ? 'border-gray-200' : 'border-slate-700'}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <button className={`flex items-center gap-2 ${theme === 'light' ? 'text-gray-600 hover:text-gray-900' : 'text-gray-400 hover:text-white'}`}>
                        ...
                        <button className={`${theme === 'light' ? 'bg-gray-100 text-gray-700 hover:bg-gray-200' : 'bg-slate-700 text-gray-300 hover:bg-slate-600'} px-4 py-2 rounded-lg transition-colors`}>
