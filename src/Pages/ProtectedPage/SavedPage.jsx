...
export default function SavedPage() {
  ...
  const displayedPosts = savedPosts.filter(post => bookmarkedPosts.has(post.id));
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-slate-600 dark:text-slate-400">
            {displayedPosts.length} {displayedPosts.length === 1 ? 'post' : 'posts'} saved for later
          </p>
        </div>
        ...
        {displayedPosts.length > 0 ? (
          <div className="space-y-6">
            {displayedPosts.map((post) => {
              ...
              const showExcerpt = post.excerpt && post.excerpt.length > 100;
          ...
          <div className="text-center py-20 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700">
            ...
            <button onClick={() => nav("/home")} className="px-6 py-2.5 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-colors shadow-sm">
              Explore Posts
            </button>
          </div>
        )}
