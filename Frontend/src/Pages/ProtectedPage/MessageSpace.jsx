...
function Bubble({ msg, isLight, prevMine, currentUserId, currentUser, otherUser, onDelete, onEdit }) {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className={`relative flex flex-col max-w-[72%] sm:max-w-[65%] ${isMine ? "items-end" : "items-start"}`}>
...
function DateDivider({ label, isLight }) {
  return (
    <div className="flex items-center gap-3 my-6">
      ...
      <span className={`text-[10px] font-semibold uppercase tracking-[0.1em] px-3 py-1 rounded-full
    ...
    >
      <div className="relative flex-shrink-0">
        <img src={av(conv.user)} alt={conv.user.full_name}
          className={`w-12 h-12 rounded-2xl object-cover ring-2 ${isLight ? "ring-gray-200" : "ring-slate-700"}`}
        />
        {conv.user.online && (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-0.5">
          ...
          {conv.user.relation && (
            <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-md flex-shrink-0 ${rColors[conv.user.relation]}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col items-end flex-shrink-0 gap-1">
            ...
            {conv.last_message?.unread > 0 && (
              <span className="w-4.5 h-4.5 min-w-[18px] px-1 rounded-full bg-blue-600 text-white text-[9px] font-bold flex items-center justify-center">
...
function EmptyState({ theme }) {
  const isLight = theme === "light";
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-5 p-8 text-center select-none">
      ...
      <div>
        ...
        <p className={`text-sm max-w-[240px] leading-relaxed ${isLight ? "text-gray-400" : "text-slate-500"}`}>
  ...
  return (
    <>
      <style>{`
        ...
        .msgs-scroll::-webkit-scrollbar-thumb { background: rgba(148,163,184,0.3); border-radius: 4px; }

        .convs-scroll::-webkit-scrollbar { width: 3px; }
        .convs-scroll::-webkit-scrollbar-track { background: transparent; }
        .convs-scroll::-webkit-scrollbar-thumb { background: rgba(148,163,184,0.2); border-radius: 3px; }
      `}</style>
      ...
      <div className={`fixed inset-0 pt-16 md:pt-20 flex flex-col ${isLight ? "bg-gray-100" : "bg-slate-950"}`}>
        <div className="flex-1 flex flex-col p-0 sm:p-3 md:p-4 lg:p-6 max-w-7xl mx-auto w-full min-h-0">
          ...
          >
            ...
            `}>
              {/* Sidebar header */}
              <div className={`px-4 sm:px-5 pt-4 pb-3 border-b flex-shrink-0
                ${isLight ? "border-gray-100" : "border-slate-700/50"}`}>
                <div className="flex items-center justify-between mb-3.5">
                  <div className="flex items-center gap-2.5">
                    <h1 className={`font-bold text-xl tracking-tight ${isLight ? "text-gray-900" : "text-white"}`}>
                      Messages
                    </h1>
                    {totalUnread > 0 && (
              ...
              {!activeConv ? (
                ...
                <>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => setMobileView("list")}
                        className={`sm:hidden p-2 -ml-1 rounded-xl transition-colors
                      ...
                      <div className="relative">
                        <img src={av(activeConv.user)} alt={activeConv.user.full_name}
                          className={`w-9 h-9 rounded-2xl object-cover ring-2 ${isLight ? "ring-gray-200" : "ring-slate-700"}`}
                        />
                        {activeConv.user.online && (
                          <span className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full bg-emerald-500 ring-2 ${isLight ? "ring-white" : "ring-slate-900"}`} />
                        )}
                      </div>

                      <div>
                        <p className={`font-bold text-sm cursor-pointer hover:underline leading-tight
                    ...
                    <div className="flex items-center gap-0.5">
                      ...
                      <button className={`w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-xl transition-colors
                        ${isLight ? "hover:bg-gray-100 text-gray-500" : "hover:bg-slate-800 text-slate-400"}`}>
                  ...
                  <div ref={containerRef} onScroll={handleScroll}
                    className={`flex-1 overflow-y-auto msgs-scroll px-4 sm:px-6 py-4 pb-6 sm:pb-4
                  ...
                  {/* Input */}
                  <div className={`px-3 sm:px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom,0px))] sm:pb-3 border-t flex-shrink-0
                    ${isLight ? "bg-white border-gray-100" : "bg-slate-900 border-slate-700/50"}`}>
                    {isRecording ? (
                      ...
                      <div className="relative">
                        ...
                        <div className={`flex items-end gap-1.5 px-2.5 py-2 rounded-2xl border transition-all
                          ${isLight
