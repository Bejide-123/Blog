    ...
    try {
      ...
      const transformedNotifications = realNotifications.map((notif) => {
        ...
        const getMessage = () => {
          ...
          return {
            primary: getDefaultMessage(notif.type, username),
            secondary: null,
          };
        };
    ...
    >
      ...
      >
        ...
        <div className="flex justify-center pt-3 pb-2">
          <div
            className={`w-12 h-1 rounded-full ${theme === "light" ? "bg-gray-300" : "bg-slate-600"}`}
          />
        </div>

        {/* Header */}
        <div
          className={`flex items-center justify-between px-5 py-3 border-b ${theme === "light" ? "border-gray-200 bg-gradient-to-r from-blue-50/50 to-purple-50/50" : "border-slate-700 bg-gradient-to-r from-blue-900/10 to-purple-900/10"}`}
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p
                className={`text-lg font-bold ${theme === "light" ? "text-slate-900" : "text-white"}`}
              >
                Notifications
              </p>
              {unreadCount > 0 && (
                <p
                  className={`text-xs ${theme === "light" ? "text-gray-600" : "text-gray-400"} font-medium`}
                >
        ...
        {unreadCount > 0 && (
          <div
            className={`px-4 py-2 border-b ${theme === "light" ? "border-gray-200" : "border-slate-700"}`}
          >
            <button
              className={`w-full py-2 ${theme === "light" ? "bg-blue-100 text-blue-600 hover:bg-blue-200" : "bg-blue-900/30 text-blue-400 hover:bg-blue-900/50"} text-sm font-semibold rounded-xl transition-colors flex items-center justify-center gap-2`}
            >
          ...
          {isLoading ? (
            ...
            <div className="p-3">
              {Array.from({ length: 5 }).map((_, index) => (
                ...
                >
                  <div
                    className={`w-12 h-12 rounded-full ${theme === "light" ? "bg-gray-200" : "bg-slate-700"} mr-3 flex-shrink-0`}
                  />
                  <div className="flex-1">
                    <div
                      className={`w-full h-3 rounded ${theme === "light" ? "bg-gray-200" : "bg-slate-700"} mb-2`}
                    />
                    <div
                      className={`w-3/4 h-3 rounded ${theme === "light" ? "bg-gray-200" : "bg-slate-700"} mb-2`}
                    />
                    <div
                      className={`w-1/3 h-2 rounded ${theme === "light" ? "bg-gray-200" : "bg-slate-700"}`}
                    />
                  </div>
                </div>
              ))}
              ...
              {notifications.map((n, index) => (
                ...
                >
                  ...
                  <div className="relative mr-3 flex-shrink-0">
                    ...
                    />
                    <div
                      className={`absolute -bottom-1 -right-1 p-1 rounded-full ${theme === "light" ? "bg-white" : "bg-slate-800"} shadow-md ring-2 ${theme === "light" ? "ring-white" : "ring-slate-800"}`}
                    >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {n.secondary && (
                      <p
                        className={`text-xs ${theme === "light" ? "text-gray-600" : "text-gray-400"} mt-1 line-clamp-2`}
                      >
                        "{n.secondary}"
                      </p>
                    )}
                  ...
                  {/* Chevron icon */}
                  <ChevronRight
                    className={`w-5 h-5 ${theme === "light" ? "text-gray-400" : "text-slate-500"} flex-shrink-0 ml-2`}
                  />
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-16 px-6">
              <div
                className={`w-20 h-20 mb-5 ${theme === "light" ? "bg-gradient-to-br from-blue-100 to-purple-100" : "bg-gradient-to-br from-blue-900/30 to-purple-900/30"} rounded-full flex items-center justify-center`}
              >
                <Bell
                  className={`w-10 h-10 ${theme === "light" ? "text-blue-600" : "text-blue-400"}`}
                />
              </div>
              <p
                className={`text-lg font-bold ${theme === "light" ? "text-slate-900" : "text-white"} mb-2`}
              >
                All caught up!
              </p>
              <p
        ...
        {notifications.length > 0 && (
          <div
            className={`border-t ${theme === "light" ? "border-gray-200 bg-white" : "border-slate-700 bg-slate-800"} p-4`}
          >
            <button
              onClick={() => {
